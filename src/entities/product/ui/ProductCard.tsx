...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <div className="group flex flex-col bg-zinc-950 border border-zinc-800/80 hover:border-zinc-600 rounded transition-all duration-200 overflow-hidden relative">
      {/* Cover Image */}
      <Link to={`/catalog/${product.id}`} className="block relative aspect-[3/4] overflow-hidden bg-zinc-900">
        <SafeImage
        ...
        <button
          ...
          className="absolute top-2.5 right-2.5 p-1.5 rounded bg-black/60 backdrop-blur-sm border border-zinc-700/60 text-zinc-300 hover:text-white hover:border-zinc-500 transition-colors"
        >
          <Heart className={`w-3.5 h-3.5 ${isLiked ? 'text-white fill-white' : ''}`} />
        </button>
      ...
      <div className="flex flex-col flex-1 p-4">
        <div className="flex items-center justify-between gap-2 text-xs text-zinc-400 mb-1.5">
          ...
          <div className="flex items-center gap-1 shrink-0 text-zinc-300">
            ...
            <span className="text-[11px] font-mono">{product.rating.toFixed(1)}</span>
          </div>
        </div>

        <Link to={`/catalog/${product.id}`} className="block mb-3">
          <h3 className="font-semibold text-sm text-zinc-100 line-clamp-2 group-hover:text-zinc-300 transition-colors leading-snug">
            {product.title}
          </h3>
        </Link>
        ...
        <div className="mt-auto pt-3 border-t border-zinc-800/80 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="p-2 text-zinc-300 hover:text-zinc-950 hover:bg-white border border-zinc-700 hover:border-white rounded transition-all disabled:opacity-20 disabled:pointer-events-none"

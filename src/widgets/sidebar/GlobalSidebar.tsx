...
export const GlobalSidebar: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      {/* Drawer */}
      <aside
        className="relative w-80 max-w-[85vw] bg-[#0a0a0c] border-r border-zinc-800 text-zinc-100 h-full flex flex-col justify-between shadow-2xl z-10 animate-in slide-in-from-left duration-200 overflow-hidden"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
          ...
          >
            <div className="w-8 h-8 rounded bg-zinc-900 border border-zinc-800 text-white flex items-center justify-center font-bold">
              <BookOpen className="w-4 h-4" />
            </div>
            <div className="flex flex-col">
              <span className="text-sm font-bold tracking-tight text-white group-hover:text-zinc-300 transition-colors uppercase">
                QazaqMarket
              </span>
          ...
          <button
            onClick={closeSidebar}
            className="p-1.5 rounded text-zinc-400 hover:text-white hover:bg-zinc-900 border border-transparent hover:border-zinc-800 transition-colors"
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-6">
            ...
            <nav className="space-y-1 text-sm">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span>{lang === 'kk' ? 'Тауарлар каталогы' : 'Каталог книг'}</span>
                </div>
              ...
              >
                ...
                {wishlistCount > 0 && (
                  <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-zinc-100 text-zinc-950 font-bold">
                    {wishlistCount}
                  </span>
                )}
              </NavLink>
              ...
              >
                ...
                {cartCount > 0 && (
                  <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-white text-zinc-950 font-bold">
                    {cartCount}
                  </span>
                )}
              </NavLink>
              ...
              {isAuthenticated && isSeller && (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 py-2 rounded text-xs font-medium transition-colors ${
                      isActive
                        ? 'bg-zinc-900 text-emerald-400 border border-emerald-900/50 font-semibold'
                        : 'text-zinc-400 hover:bg-zinc-900/60 hover:text-emerald-400'
                    }`
                  }
                >
          ...
          <div className="grid grid-cols-2 gap-2 p-2 bg-zinc-900/60 rounded border border-zinc-800">
            {/* Language Switcher */}
            <div className="flex items-center justify-center p-0.5 bg-zinc-950 rounded border border-zinc-800">
            ...
            <button
              onClick={toggleTheme}
              className="flex items-center justify-center gap-1.5 py-1 px-2 text-xs font-medium bg-zinc-950 text-zinc-300 border border-zinc-800 rounded hover:bg-zinc-900 transition-colors"
            >
              {theme === 'dark' ? (
                <>
                  <Moon className="w-3.5 h-3.5 text-zinc-300" />
                  <span className="text-[11px]">{lang === 'kk' ? 'Күңгірт' : 'Тёмная'}</span>
                </>
              ) : (
                <>
                  <Sun className="w-3.5 h-3.5 text-amber-400" />
                  <span className="text-[11px]">{lang === 'kk' ? 'Жарық' : 'Светлая'}</span>
                </>
              )}
            </button>
          </div>
              ...
              {CATEGORIES.map((cat) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-3 py-2 text-left rounded hover:bg-zinc-900 border border-transparent hover:border-zinc-800 transition-colors group"
                  >
                    ...
                    <ChevronRight className="w-3.5 h-3.5 text-zinc-400 group-hover:text-white shrink-0 transition-transform group-hover:translate-x-0.5" />
                  </button>
                );
              })}
            </div>
        ...
        <div className="p-4 border-t border-zinc-800 bg-zinc-950/80">
          {isAuthenticated && user ? (
            <div className="flex items-center justify-between gap-3">
              ...
              >
                <div className="w-8 h-8 rounded-full bg-zinc-100 text-zinc-950 flex items-center justify-center font-bold text-xs shrink-0">
              ...
              <button
                ...
                className="p-2 text-zinc-400 hover:text-rose-400 hover:bg-zinc-900 rounded border border-transparent hover:border-zinc-800 transition-colors"
                title={t('nav.logout')}
                aria-label="Выйти"
              >
            ...
            <Link
              to="/auth"
              onClick={closeSidebar}
              className="w-full flex items-center justify-center gap-2 py-2 px-4 bg-white hover:bg-zinc-200 text-zinc-950 text-xs font-semibold rounded transition-colors"
            >

...
export const Sidebar: React.FC<SidebarProps> = ({ type }) => {
  ...
  const clientLinks: SidebarLinkItem[] = [
    { to: '/client/dashboard', label: 'Главный кабинет', icon: LayoutDashboard },
    { to: '/client/catalog', label: 'Каталог книг', icon: BookOpen },
    { to: '/client/cart', label: 'Корзина', icon: ShoppingBag, badge: cartCount > 0 ? cartCount : undefined },
    { to: '/client/orders', label: 'Мои заказы', icon: Clock },
    { to: '/client/wallet', label: 'Kaspi Кошелек', icon: Wallet },
  ];

  const adminLinks: SidebarLinkItem[] = [
    { to: '/admin/dashboard', label: 'Сводка и метрики', icon: LayoutDashboard },
    { to: '/admin/catalog', label: 'Управление товарами', icon: Layers },
    { to: '/admin/warehouse', label: 'Склад и остатки', icon: Package },
    { to: '/admin/analytics', label: 'Аналитика продаж', icon: BarChart3 },
  ];
          ...
          {links.map((link) => {
            ...
            return (
              <NavLink
                ...
                to={link.to}
                className={({ isActive }) =>
                  `flex items-center justify-between px-2.5 py-1.5 rounded-md text-xs font-medium ${
                    isActive
